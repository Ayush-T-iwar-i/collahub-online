  ...
  const loadAll = async (isRefresh = false) => {
    ...
    try {
      const [assignedRes, availRes, reqRes] = await Promise.allSettled([
        API.get("/subject-requests/my-subjects"),     // ✅ accepted + timetable
        API.get("/subjects/for-teacher"),              // available to request
        API.get("/subject-requests/my"),              // all requests with status
      ]);
