// ============================================================
// FILE 1: app/(auth)/teacher-forgot.js
// ============================================================
...
export default function TeacherForgot() {
  ...
  useEffect(() => {
    let interval;
    if (step === 2 && timer > 0) interval = setInterval(() => setTimer((p) => p - 1), 1000);
    if (timer === 0) setOtpExpired(true);
    return () => clearInterval(interval);
  }, [step, timer]);

  const animatedCard = useAnimatedStyle(() => ({ opacity: opacity.value, transform: [{ translateY: translateY.value }] }));
  const animatedBg = useAnimatedStyle(() => ({ transform: [{ scale: scale.value }] }));
  ...
  const handleSendOtp = async () => {
    ...
    try {
      ...
      setStep(2); setTimer(300); setOtpExpired(false); setOtp(["","","","","",""]);
  ...
  const handleVerifyOtp = async () => {
    if (otp.join("").length !== 6) return Alert.alert("Error","Enter full 6 digit OTP");
  ...
  const handleResetPassword = async () => {
    ...
    try {
      setLoading(true);
      await API.post("/auth/reset-password", { email: email.trim().toLowerCase(), otp: otp.join(""), newPassword });
      Alert.alert("Success 🎉","Password reset successfully!", [{ text: "Login", onPress: () => router.replace("/(auth)/teacher-login") }]);
  ...
  return (
    <View style={{ flex: 1 }}>
      ...
      <LinearGradient colors={["rgba(15,10,0,0.85)","rgba(0,0,0,0.92)"]} style={StyleSheet.absoluteFillObject} />
      <Animated.View style={[styles.container, animatedCard]}>
        <BlurView intensity={90} tint="dark" style={styles.card}>
          <View style={[styles.iconCircle, { backgroundColor: accentColor+"18", borderColor: accentColor+"33" }]}>
            <Ionicons name="lock-open-outline" size={30} color={accentColor} />
          </View>
          ...
          <View style={styles.steps}>
            {stepLabels.map((label, i) => {
              const isActive = i+1===step; const isDone = i+1<step;
              return (
                <React.Fragment key={i}>
                  <View style={styles.stepItem}>
                    <View style={[styles.stepCircle, isDone && { backgroundColor: "#34d399", borderColor: "#34d399" }, isActive && { backgroundColor: accentColor+"22", borderColor: accentColor }]}>
                      {isDone ? <Ionicons name="checkmark" size={14} color="#fff" /> : <Text style={styles.stepNum}>{i+1}</Text>}
                    </View>
                    <Text style={[styles.stepLabel, isActive && { color: accentColor }]}>{label}</Text>
                  </View>
          ...
          {step===1 && (
            <>
              ...
              <Pressable style={styles.btn} onPress={handleSendOtp} disabled={loading}>
                <LinearGradient colors={[accentColor, "#d97706"]} start={{x:0,y:0}} end={{x:1,y:0}} style={styles.btnGrad}>
          ...
          {step===2 && (
            <>
              <View style={[styles.emailBadge, { backgroundColor: accentColor+"10", borderColor: accentColor+"25" }]}>
                <Ionicons name="mail" size={14} color={accentColor} />
                <Text style={[styles.emailBadgeText, { color: "#94a3b8" }]}>{email}</Text>
              </View>
              <View style={styles.otpRow}>
                {otp.map((digit, index) => (
                  <TextInput key={index} ref={(ref) => (inputRefs.current[index]=ref)}
                    style={[styles.otpBox, { borderColor: digit ? accentColor : "rgba(255,255,255,0.12)" }]}
              ...
              <Text style={styles.timerText}>{otpExpired ? "⚠️ OTP Expired" : `Expires in ${formatTime()}`}</Text>
              {otpExpired ? (
                <Pressable style={styles.btn} onPress={handleSendOtp} disabled={loading}>
          ...
          {step===3 && (
            <>
              <View style={styles.inputWrapper}>
                ...
                <TextInput placeholder="New Password" placeholderTextColor="#888" style={[styles.input,{flex:1}]}
              ...
              <View style={styles.inputWrapper}>
                ...
                <TextInput placeholder="Confirm Password" placeholderTextColor="#888" style={[styles.input,{flex:1}]}
              ...
              {confirmPassword.length > 0 && (
                <View style={styles.matchRow}>
                  ...
                  <Text style={{ color: newPassword===confirmPassword?"#34d399":"#f87171", fontSize:12, marginLeft:6 }}>
...
const styles = StyleSheet.create({
  ...
  card: { width: width > 500 ? 460 : "100%", padding: 32, borderRadius: 28, backgroundColor: "rgba(255,255,255,0.05)", borderWidth: 1, borderColor: "rgba(255,255,255,0.1)", overflow: "hidden" },
  iconCircle: { width: 60, height: 60, borderRadius: 30, justifyContent: "center", alignItems: "center", alignSelf: "center", marginBottom: 16, borderWidth: 1 },
  title: { fontSize: 24, fontWeight: "800", color: "#fff", textAlign: "center", letterSpacing: 0.5 },
  subtitle: { color: "#64748b", fontSize: 13, textAlign: "center", marginTop: 6, marginBottom: 24 },
  steps: { flexDirection: "row", alignItems: "center", justifyContent: "center", marginBottom: 28 },
  stepItem: { alignItems: "center", gap: 4 },
  stepCircle: { width: 30, height: 30, borderRadius: 15, backgroundColor: "#1a2535", justifyContent: "center", alignItems: "center", borderWidth: 1, borderColor: "rgba(255,255,255,0.1)" },
  stepNum: { color: "#64748b", fontSize: 12, fontWeight: "700" },
  stepLabel: { color: "#374151", fontSize: 10, fontWeight: "600" },
  stepLine: { width: 40, height: 1, backgroundColor: "rgba(255,255,255,0.08)", marginHorizontal: 6, marginBottom: 16 },
  inputWrapper: { flexDirection: "row", alignItems: "center", backgroundColor: "rgba(255,255,255,0.07)", borderRadius: 14, marginBottom: 14, borderWidth: 1, borderColor: "rgba(255,255,255,0.1)", paddingHorizontal: 14 },
  ...
  emailBadge: { flexDirection: "row", alignItems: "center", gap: 8, padding: 12, borderRadius: 12, marginBottom: 20, borderWidth: 1 },
  emailBadgeText: { fontSize: 13 },
  otpRow: { flexDirection: "row", justifyContent: "space-between", marginBottom: 12 },
  otpBox: { width: 44, height: 54, backgroundColor: "rgba(255,255,255,0.07)", borderRadius: 12, textAlign: "center", fontSize: 22, color: "#fff", borderWidth: 1.5 },
  timerText: { color: "#64748b", fontSize: 12, textAlign: "center", marginBottom: 14 },
  matchRow: { flexDirection: "row", alignItems: "center", marginBottom: 14, marginTop: -6 },
