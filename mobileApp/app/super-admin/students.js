...
export default function SuperAdminStudents() {
  ...
  const filtered=students.filter(s=>{
    ...
    return s.name?.toLowerCase().includes(q)||s.email?.toLowerCase().includes(q)||s.studentId?.toLowerCase().includes(q)||s.department?.toLowerCase().includes(q);
  });
  ...
  const del=(item)=>Alert.alert("Delete",`"${item.name}" delete karo?`,[{text:"Cancel",style:"cancel"},{text:"Delete",style:"destructive",onPress:async()=>{
    try{ await API.delete(`/super-admin/users/${item._id}`); setStudents(p=>p.filter(s=>s._id!==item._id)); setTotal(t=>t-1); }catch(e){Alert.alert("Error",e.response?.data?.message||"Failed");}
  }}]);
  ...
  return(
    <View style={s.container}>
      ...
      <LinearGradient colors={["#070d1a","#0a1628"]} style={s.header}>
        ...
        <Pressable onPress={()=>load(1,true)} style={s.refreshBtn}><Ionicons name="refresh" size={18} color="#34d399"/></Pressable>
      </LinearGradient>

      <View style={s.searchRow}>
        <Ionicons name="search" size={14} color="#374151"/>
        <TextInput style={s.searchInput} placeholder="Search name, ID, department..." placeholderTextColor="#374151" value={search} onChangeText={setSearch}/>
      ...
      <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={s.filterRow}>
        {["all",...colleges].map(c=>(
          <Pressable key={c} onPress={()=>{setCollege(c);}} style={[s.chip,college===c&&s.chipA]}>
            <Text style={[s.chipT,college===c&&{color:"#34d399"}]}>{c==="all"?"All Colleges":c.split(" ").slice(0,2).join(" ")}</Text>
          </Pressable>
        ))}
      </ScrollView>
      ...
      {loading?<ActivityIndicator size="large" color="#34d399" style={{marginTop:50}}/>:(
        <FlatList data={filtered} keyExtractor={i=>i._id} contentContainerStyle={{padding:16,paddingBottom:40}} showsVerticalScrollIndicator={false}
          onEndReached={()=>{if(students.length<total)load(page+1);}} onEndReachedThreshold={0.4}
          ListFooterComponent={fetching&&!loading?<ActivityIndicator color="#34d399" style={{marginVertical:14}}/>:null}
          ListEmptyComponent={<View style={s.empty}><Ionicons name="people-outline" size={44} color="#1f2937"/><Text style={s.emptyT}>No students found</Text></View>}
            ...
            return(
              <View style={s.card}>
                ...
                <View style={{flex:1}}>
                  ...
                  <View style={s.tags}>
                    {item.studentId&&<View style={[s.tag,{borderColor:color+"44",backgroundColor:color+"12"}]}><Text style={[s.tagT,{color}]}>{item.studentId}</Text></View>}
                    {item.department&&<View style={[s.tag,{borderColor:"#34d39940",backgroundColor:"#34d39912"}]}><Text style={[s.tagT,{color:"#34d399"}]} numberOfLines={1}>{item.department.split("(")[0].trim()}</Text></View>}
                    {item.section&&<View style={[s.tag,{borderColor:"#60a5fa40",backgroundColor:"#60a5fa12"}]}><Text style={[s.tagT,{color:"#60a5fa"}]}>Sec {item.section}</Text></View>}
