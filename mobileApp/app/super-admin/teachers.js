...
export default function SuperAdminTeachers(){
  ...
  const del=(item)=>Alert.alert("Delete",`"${item.name}" delete karo?`,[{text:"Cancel",style:"cancel"},{text:"Delete",style:"destructive",onPress:async()=>{
    try{await API.delete(`/super-admin/users/${item._id}`);setTeachers(p=>p.filter(t=>t._id!==item._id));setTotal(t=>t-1);}catch(e){Alert.alert("Error",e.response?.data?.message||"Failed");}
  }}]);
  ...
  return(
    <View style={s.container}>
      ...
      <LinearGradient colors={["#070d1a","#0a1628"]} style={s.header}>
        ...
        <Pressable onPress={()=>load(1,true)} style={s.refreshBtn}><Ionicons name="refresh" size={18} color="#f59e0b"/></Pressable>
      </LinearGradient>
      <View style={s.searchRow}>
        <Ionicons name="search" size={14} color="#374151"/>
        <TextInput style={s.searchInput} placeholder="Search name, email, department..." placeholderTextColor="#374151" value={search} onChangeText={setSearch}/>
      ...
      <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={s.filterRow}>
        {["all",...colleges].map(c=>(
          <Pressable key={c} onPress={()=>setCollege(c)} style={[s.chip,college===c&&s.chipA]}>
            <Text style={[s.chipT,college===c&&{color:"#f59e0b"}]}>{c==="all"?"All Colleges":c.split(" ").slice(0,2).join(" ")}</Text>
          </Pressable>
        ))}
      </ScrollView>
      {loading?<ActivityIndicator size="large" color="#f59e0b" style={{marginTop:50}}/>:(
        <FlatList data={filtered} keyExtractor={i=>i._id} contentContainerStyle={{padding:16,paddingBottom:40}} showsVerticalScrollIndicator={false}
          onEndReached={()=>{if(teachers.length<total)load(page+1);}} onEndReachedThreshold={0.4}
          ListFooterComponent={fetching&&!loading?<ActivityIndicator color="#f59e0b" style={{marginVertical:14}}/>:null}
          ListEmptyComponent={<View style={s.empty}><Ionicons name="person-outline" size={44} color="#1f2937"/><Text style={s.emptyT}>No teachers found</Text></View>}
          renderItem={({item,index})=>{
            const color=ac(item.name||"");
            return(
              <View style={s.card}>
                <Text style={s.idx}>#{index+1}</Text>
                <View style={[s.av,{backgroundColor:color+"20",borderColor:color+"50"}]}><Text style={[s.avT,{color}]}>{ini(item.name)}</Text></View>
                <View style={{flex:1}}>
                  <Text style={s.name} numberOfLines={1}>{item.name}</Text>
                  <Text style={s.email} numberOfLines={1}>{item.email}</Text>
                  <View style={s.tags}>
                    {item.department&&<View style={[s.tag,{borderColor:"#f59e0b40",backgroundColor:"#f59e0b12"}]}><Text style={[s.tagT,{color:"#f59e0b"}]} numberOfLines={1}>{item.department.split("(")[0].trim()}</Text></View>}
                    {item.college&&<View style={[s.tag,{borderColor:"#00c6ff40",backgroundColor:"#00c6ff12"}]}><Text style={[s.tagT,{color:"#00c6ff"}]} numberOfLines={1}>{item.college.split(" ").slice(0,2).join(" ")}</Text></View>}
                  </View>
                </View>
...
const s=StyleSheet.create({
  ...
  card:{flexDirection:"row",alignItems:"center",backgroundColor:"#0f1b2d",borderRadius:14,padding:12,marginBottom:8,gap:10,borderWidth:1,borderColor:"rgba(255,255,255,0.05)"},
  ...
  avT:{fontSize:13,fontWeight:"800"},name:{color:"#fff",fontSize:13,fontWeight:"700"},
  email:{color:"#64748b",fontSize:11,marginTop:2},tags:{flexDirection:"row",gap:6,marginTop:5,flexWrap:"wrap"},
  tag:{paddingHorizontal:7,paddingVertical:2,borderRadius:7,borderWidth:1},tagT:{fontSize:9,fontWeight:"700"},
