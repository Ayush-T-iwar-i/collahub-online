// app/admin/manage-subjects.js  — FIXED
...
const DEPT_COLORS = { CSE:"#00c6ff",ECE:"#a78bfa",ME:"#f59e0b",CE:"#34d399",IT:"#f87171",EE:"#60a5fa",AI:"#fb923c",DATA:"#34d399" };
const getColor = (dept="") => {
    ...
    {TYPES.map(t => {
      ...
      return (
        <Pressable key={t}
          ...
          onPress={() => onChange(t)}>
          <Ionicons name={t==="Theory"?"book-outline":t==="Lab"?"flask-outline":"layers-outline"} size={13} color={sel?tc:"#64748b"} />
    ...
    <View style={styles.card}>
      ...
      <View style={styles.cardBody}>
        ...
        <View style={styles.cardMetaRow}>
          {item.code && <View style={[styles.codeBadge,{backgroundColor:color+"20"}]}><Text style={[styles.codeBadgeText,{color}]}>{item.code}</Text></View>}
          <View style={styles.semBadge}><Text style={styles.semBadgeText}>Sem {item.semester}</Text></View>
          <View style={[styles.typeBadge,{backgroundColor:typeColor+"25"}]}><Text style={[styles.typeBadgeText,{color:typeColor}]}>{item.type??"Theory"}</Text></View>
        </View>
        <Text style={styles.cardDept} numberOfLines={1}>{deptShort}</Text>
      </View>
      <View style={styles.cardActions}>
        <Pressable style={styles.editBtn} onPress={() => onEdit(item)}><Ionicons name="pencil" size={13} color="#f59e0b" /></Pressable>
        <Pressable style={styles.deleteBtn} onPress={() => onDelete(item)}><Ionicons name="trash" size={13} color="#f87171" /></Pressable>
      </View>
    </View>
  );
};
  ...
  <View style={styles.stepRow}>
    {Array.from({length:total}).map((_,i) => (
      <React.Fragment key={i}>
        <View style={[styles.stepDot,i<current&&{backgroundColor:"#34d399"},i===current&&{backgroundColor:"#34d399",transform:[{scale:1.2}]},i>current&&{backgroundColor:"rgba(255,255,255,0.12)"}]}>
          {i < current
            ? <Ionicons name="checkmark" size={10} color="#000" />
            : <Text style={[styles.stepNum,{color:i===current?"#000":"#64748b"}]}>{i+1}</Text>}
        </View>
        {i < total-1 && <View style={[styles.stepLine,{backgroundColor:i<current?"#34d399":"rgba(255,255,255,0.08)"}]} />}
      </React.Fragment>
    ))}
  </View>
);
...
export default function ManageSubjects() {
  ...
  const [selType,      setSelType]      = useState("All"); // All | Theory | Lab
  ...
  useFocusEffect(useCallback(() => {
    (async () => { await loadCollegeInfo(); loadSubjects(); })();
  }, []));
  ...
  const loadSubjects = async (isRefresh=false) => {
    try {
      ...
      setSubjects(data);
      applyFilter(data, search, selDept, selSem, selType);
    } catch {
      Alert.alert("Error","Could not load subjects.");
    } finally { setLoading(false); setRefreshing(false); }
  };

  const applyFilter = (data=subjects, q=search, dept=selDept, sem=selSem, typ=selType) => {
    let r = [...data];
    if (q.trim()) {
      const lq = q.toLowerCase();
      r = r.filter(s => s.name?.toLowerCase().includes(lq) || s.code?.toLowerCase().includes(lq) || s.department?.toLowerCase().includes(lq));
    }
    if (dept !== "All") r = r.filter(s => s.department === dept);
    if (sem  !== "All") r = r.filter(s => String(s.semester) === sem);
    if (typ  !== "All") {
      r = r.filter(s => {
        const t = (s.type||"theory").toLowerCase();
        if (typ === "Theory") return t !== "lab" && t !== "practical";
        if (typ === "Lab")    return t === "lab" || t === "practical";
        return true;
      });
    }
    setFiltered(r);
  ...
  const openWizard = () => { setStep(0);setWizDept("");setWizSem("");setSubjectCount("");setSubjectForms([]);setWizVisible(true); };
  ...
  const updateForm = (idx,key,val) => setSubjectForms(prev=>{const a=[...prev];a[idx]={...a[idx],[key]:val};return a;});

  const handleWizardSave = async () => {
    for (let i=0;i<subjectForms.length;i++) {
      if (!subjectForms[i].name?.trim()) return Alert.alert("Error",`Subject ${i+1}: Name is required.`);
      if (!subjectForms[i].code?.trim()) return Alert.alert("Error",`Subject ${i+1}: Code is required.`);
    }
    setSaving(true);
    let ok=0,fail=0,msg="";
    for (const s of subjectForms) {
      try {
        await API.post("/subjects/create",{name:s.name.trim(),code:s.code.trim().toUpperCase(),type:s.type||"Theory",college:adminCollege,department:wizDept,semester:Number(wizSem),credits:Number(s.credits)||0,description:s.description?.trim()||""});
        ok++;
      } catch(e) { fail++; msg=e.response?.data?.message||"Error"; }
    }
    setSaving(false); setWizVisible(false); loadSubjects();
    Alert.alert("Done!", fail>0?`${ok} added, ${fail} failed.\n${msg}`:`${ok} subject${ok>1?"s":""} added successfully!`);
  };

  const openEdit = (s) => { setEditingSubject(s);setEditForm({name:s.name||"",code:s.code||"",type:s.type||"Theory",credits:String(s.credits||""),description:s.description||""});setEditModal(true); };

  const handleEditSave = async () => {
    if (!editForm.name.trim()) return Alert.alert("Error","Subject name is required.");
    if (!editForm.code.trim()) return Alert.alert("Error","Subject code is required.");
    setSaving(true);
    try {
      await API.put(`/subjects/${editingSubject._id}`,{name:editForm.name.trim(),code:editForm.code.trim().toUpperCase(),type:editForm.type||"Theory",credits:Number(editForm.credits)||0,description:editForm.description?.trim()||""});
      setEditModal(false); loadSubjects(); Alert.alert("Updated!","Subject updated successfully.");
    } catch(e) { Alert.alert("Error",e.response?.data?.message||"Could not update subject."); }
  ...
  const handleDelete = (s) => {
    Alert.alert("Delete Subject",`Delete "${s.name}"? This cannot be undone.`,[
      ...
      {text:"Delete",style:"destructive",onPress:async()=>{
        try { await API.delete(`/subjects/${s._id}`); loadSubjects(); }
        catch(e) { Alert.alert("Error",e.response?.data?.message||"Failed to delete subject."); }
      }},
    ]);
  };
  ...
  const pickExcel = async () => {
    try {
      ...
      setImportRows(rows);setImportErrors(errors);setImportStage("preview");setImportDone(0);setImportFailed([]);setImportModal(true);
    } catch(err) { Alert.alert("Error","Could not read file: "+err.message); }
  };
  ...
  const runExcelImport = async () => {
    ...
    for (let i=0;i<importRows.length;i+=20) {
      const batch=importRows.slice(i,i+20);
      for (const row of batch) {
        try {
          await API.post("/subjects/create",{name:row.name,code:row.code.toUpperCase(),type:row.type||"Theory",college:row.college,department:row.dept,semester:Number(row.sem),credits:Number(row.credits)||0});
          done++; setImportDone(done);
        } catch(e) { failed.push({row:row.name,error:e.response?.data?.message||"Failed"}); setImportFailed([...failed]); }
      }
      if (i+20<importRows.length) await new Promise(r=>setTimeout(r,300));
    }
    setImportStage("done"); loadSubjects();
  };

  const importProgress = importRows.length>0?Math.round((importDone/importRows.length)*100):0;

  const TYPE_FILTER_BTNS = [
    {key:"All",    label:"All",    icon:"apps-outline",  color:"#64748b"},
  ...
  return (
    <View style={styles.container}>
      ...
      {/* ── STICKY HEADER ── */}
      <LinearGradient colors={["#080d17","#0f1923"]} style={[styles.header, {paddingTop: insets.top + 14}]}>
        ...
        <View style={styles.headerCenter}>
          ...
          <Text style={styles.headerSub} numberOfLines={1}>
            {adminCollege?adminCollege.split(" ").slice(0,3).join(" "):"Loading..."} · {filtered.length} subjects
          </Text>
        </View>
        <View style={styles.headerRight}>
          <Pressable onPress={pickExcel} style={[styles.iconBtn,{backgroundColor:"rgba(52,211,153,0.12)",borderColor:"rgba(52,211,153,0.3)"}]}>
            <Ionicons name="document-text-outline" size={17} color="#34d399" />
          </Pressable>
          <Pressable onPress={openWizard} style={[styles.iconBtn,{backgroundColor:"rgba(52,211,153,0.12)",borderColor:"rgba(52,211,153,0.3)"}]}>
      ...
      <View style={styles.filterBar}>
        ...
        <View style={styles.searchBar}>
          ...
          <TextInput style={styles.searchInput} placeholder="Search subjects..."
            placeholderTextColor="#374151" value={search}
            onChangeText={t=>{setSearch(t);applyFilter(subjects,t,selDept,selSem,selType);}} />
          {search.length>0 && <Pressable onPress={()=>{setSearch("");applyFilter(subjects,"",selDept,selSem,selType);}}><Ionicons name="close-circle" size={15} color="#64748b" /></Pressable>}
        </View>

        {/* Dept filter */}
        <FlatList
          horizontal showsHorizontalScrollIndicator={false}
          data={["All",...collegeDepts]} keyExtractor={d=>d}
          contentContainerStyle={styles.chipRow}
          style={{marginBottom:6}}
          renderItem={({item:d}) => {
            const short=d==="All"?"All":(d.match(/\(([^)]+)\)/)?.[1]||d.split(" ")[0]);
            const act=selDept===d; const color=d==="All"?"#34d399":getColor(d);
            return (
              <Pressable style={[styles.chip,act&&{backgroundColor:color+"22",borderColor:color+"55"}]}
                onPress={()=>{setSelDept(d);applyFilter(subjects,search,d,selSem,selType);}}>
        ...
        {/* Sem + Type row */}
        <View style={styles.semTypeRow}>
          {/* Semester chips */}
          <FlatList
            horizontal showsHorizontalScrollIndicator={false}
            data={["All",...SEMESTERS]} keyExtractor={s=>s}
            contentContainerStyle={{gap:6}}
            style={{flex:1}}
            renderItem={({item:s}) => {
              const act=selSem===s;
              return (
                <Pressable style={[styles.semChip,act&&{backgroundColor:"rgba(167,139,250,0.2)",borderColor:"#a78bfa"}]}
                  onPress={()=>{setSelSem(s);applyFilter(subjects,search,selDept,s,selType);}}>
                  <Text style={[styles.chipText,act&&{color:"#a78bfa"}]}>{s==="All"?"All":`S${s}`}</Text>
                </Pressable>
              );
            }}
          />
          ...
          <View style={styles.typeFilterRow}>
            {TYPE_FILTER_BTNS.map(tb => {
              const act = selType===tb.key;
              return (
                <Pressable key={tb.key}
                  style={[styles.typeFilterBtn, act&&{backgroundColor:tb.color+(tb.key==="All"?"18":"20"),borderColor:tb.color+(tb.key==="All"?"40":"55")}]}
                  onPress={()=>{setSelType(tb.key);applyFilter(subjects,search,selDept,selSem,tb.key);}}>
                  <Ionicons name={tb.icon} size={12} color={act?tb.color:"#64748b"} />
                  <Text style={[styles.typeFilterText,act&&{color:tb.color}]}>{tb.label}</Text>
                </Pressable>
              );
            })}
          </View>
            ...
            ListEmptyComponent={() => (
              <View style={styles.emptyState}>
                ...
                <View style={styles.emptyBtns}>
                  <Pressable style={styles.emptyAddBtn} onPress={openWizard}><Ionicons name="add-circle-outline" size={15} color="#34d399" /><Text style={styles.emptyAddText}>Add Manually</Text></Pressable>
                  <Pressable style={[styles.emptyAddBtn,{borderColor:"rgba(52,211,153,0.2)"}]} onPress={pickExcel}><Ionicons name="document-text-outline" size={15} color="#34d399" /><Text style={styles.emptyAddText}>Excel Import</Text></Pressable>
                </View>
              </View>
            )}
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.sheet}>
            ...
            <View style={styles.sheetHeader}>
              {step>0?<Pressable onPress={()=>setStep(p=>Math.max(0,p-1))} style={styles.sheetBackBtn}><Ionicons name="arrow-back" size={17} color="#fff" /></Pressable>:<View style={{width:36}}/>}
              <View style={{flex:1,alignItems:"center"}}>
                <Text style={styles.sheetTitle}>{STEP_TITLES[step]}</Text>
                <Text style={styles.sheetSub}>Step {step+1} of 4</Text>
              </View>
              <Pressable onPress={()=>setWizVisible(false)} style={styles.closeBtn}><Ionicons name="close" size={19} color="#64748b" /></Pressable>
            </View>
            ...
            <ScrollView showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled" contentContainerStyle={styles.sheetBody}>
              ...
              {step===0&&(
                <>
                  ...
                  <View style={styles.chipGrid}>
                    {collegeDepts.map(d=>{
                      const sel=wizDept===d;const color=getColor(d);const short=d.match(/\(([^)]+)\)/)?.[1]||d.split(" ")[0];
                      return(
                        <Pressable key={d} style={[styles.deptChip,sel&&{backgroundColor:color+"22",borderColor:color+"66"}]} onPress={()=>setWizDept(d)}>
                  ...
                  {wizDept&&<View style={styles.selectedBadge}><Ionicons name="school" size={13} color="#34d399"/><Text style={styles.selectedBadgeText} numberOfLines={2}>{wizDept} ✓</Text></View>}
                </>
              )}
              {step===1&&(
                <>
                  ...
                  <View style={styles.semGrid}>
                    {SEMESTERS.map(s=>(
                      <Pressable key={s} style={[styles.semBox,wizSem===s&&{backgroundColor:"rgba(52,211,153,0.2)",borderColor:"#34d399"}]} onPress={()=>setWizSem(s)}>
              ...
              {step===2&&(
                <>
                  <Text style={styles.hint}>How many subjects for <Text style={{color:"#34d399"}}>{wizDept.match(/\(([^)]+)\)/)?.[1]||wizDept.split(" ")[0]} · Sem {wizSem}</Text>?</Text>
                  <View style={styles.countChips}>
                    {["1","2","3","4","5","6","7","8"].map(n=>(
                      <Pressable key={n} style={[styles.countChip,subjectCount===n&&{backgroundColor:"rgba(52,211,153,0.2)",borderColor:"#34d399"}]} onPress={()=>setSubjectCount(n)}>
                  ...
                  <View style={styles.countInputWrap}>
                    <TextInput style={styles.countInput} value={subjectCount} onChangeText={setSubjectCount} keyboardType="numeric" placeholder="e.g. 10" placeholderTextColor="#374151" maxLength={2}/>
                  ...
                  {subjectForms.map((sf,idx)=>(
                    <View key={idx} style={styles.subCard}>
                      <View style={styles.subCardHeader}>
                        <View style={styles.subNumBadge}><Text style={styles.subNum}>{idx+1}</Text></View>
                      ...
                      <View style={styles.fieldRow}><Ionicons name="book-outline" size={14} color="#64748b" style={{marginRight:8}}/><TextInput style={styles.fieldInput} value={sf.name} onChangeText={v=>updateForm(idx,"name",v)} placeholder="e.g. Data Structures" placeholderTextColor="#374151"/></View>
                      <Text style={styles.fieldLabel}>Subject Code *</Text>
                      <View style={styles.fieldRow}><Ionicons name="code-slash" size={14} color="#64748b" style={{marginRight:8}}/><TextInput style={styles.fieldInput} value={sf.code} onChangeText={v=>updateForm(idx,"code",v)} placeholder="e.g. CS301" placeholderTextColor="#374151" autoCapitalize="characters"/></View>
                      <Text style={styles.fieldLabel}>Type *</Text>
                      <TypeChips value={sf.type||"Theory"} onChange={v=>updateForm(idx,"type",v)}/>
                      <Text style={styles.fieldLabel}>Credits (optional)</Text>
                      <View style={styles.fieldRow}><Ionicons name="star-outline" size={14} color="#64748b" style={{marginRight:8}}/><TextInput style={styles.fieldInput} value={sf.credits} onChangeText={v=>updateForm(idx,"credits",v)} placeholder="e.g. 4" placeholderTextColor="#374151" keyboardType="numeric"/></View>
                    </View>
                  ))}
                </>
              )}
            ...
            <View style={styles.sheetFooter}>
              {step<3
                ?<Pressable style={styles.nextBtn} onPress={wizNext}><LinearGradient colors={["#10b981","#059669"]} start={{x:0,y:0}} end={{x:1,y:0}} style={styles.nextBtnGrad}><Text style={styles.nextBtnText}>Continue</Text><Ionicons name="arrow-forward" size={17} color="#fff"/></LinearGradient></Pressable>
                :<Pressable style={[styles.nextBtn,saving&&{opacity:0.6}]} onPress={handleWizardSave} disabled={saving}><LinearGradient colors={["#10b981","#059669"]} start={{x:0,y:0}} end={{x:1,y:0}} style={styles.nextBtnGrad}>{saving?<ActivityIndicator color="#fff"/>:<><Ionicons name="checkmark-circle" size={17} color="#fff"/><Text style={styles.nextBtnText}>Add {subjectForms.length} Subject{subjectForms.length>1?"s":""}</Text></>}</LinearGradient></Pressable>
              }
            </View>
          </View>
        </View>
      ...
      <Modal visible={importModal} transparent animationType="slide" onRequestClose={()=>setImportModal(false)}>
        <View style={styles.modalOverlay}>
          <View style={styles.sheet}>
            ...
            <View style={styles.sheetHeader}>
              <View style={{width:36}}/>
              <View style={{flex:1,alignItems:"center"}}><Text style={styles.sheetTitle}>Excel / CSV Import</Text><Text style={styles.sheetSub}>{adminCollege.split(" ").slice(0,3).join(" ")}</Text></View>
              <Pressable onPress={()=>setImportModal(false)} style={styles.closeBtn}><Ionicons name="close" size={19} color="#64748b"/></Pressable>
            </View>
            {importStage==="preview"&&(
              <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{padding:20,paddingBottom:50}}>
                <View style={styles.importStats}><View style={[styles.importStat,{borderTopColor:"#34d399"}]}><Text style={[styles.importStatNum,{color:"#34d399"}]}>{importRows.length}</Text><Text style={styles.importStatLabel}>Ready</Text></View><View style={[styles.importStat,{borderTopColor:"#f87171"}]}><Text style={[styles.importStatNum,{color:"#f87171"}]}>{importErrors.length}</Text><Text style={styles.importStatLabel}>Errors</Text></View><View style={[styles.importStat,{borderTopColor:"#f59e0b"}]}><Text style={[styles.importStatNum,{color:"#f59e0b"}]}>{importRows.length+importErrors.length}</Text><Text style={styles.importStatLabel}>Total</Text></View></View>
                <View style={styles.importHintBox}><Text style={styles.importHintTitle}>Required Excel Columns</Text><Text style={styles.importHintReq}>name  ·  code  ·  department  ·  semester</Text><Text style={styles.importHintOpt}>type  ·  credits  (optional)</Text></View>
                {importRows.length>0&&(<><Text style={styles.sectionHead}>Ready ({importRows.length})</Text>{importRows.slice(0,5).map((r,i)=>(<View key={i} style={styles.previewRowCard}><Text style={styles.previewRowName}>{r.name}</Text><View style={styles.previewRowTags}><View style={styles.previewTag}><Text style={styles.previewTagText}>{r.code}</Text></View><View style={styles.previewTag}><Text style={styles.previewTagText}>Sem {r.sem}</Text></View><View style={styles.previewTag}><Text style={styles.previewTagText}>{r.type}</Text></View></View></View>))}{importRows.length>5&&<Text style={styles.moreText}>...and {importRows.length-5} more</Text>}<Pressable style={styles.importBtn} onPress={runExcelImport}><LinearGradient colors={["#10b981","#059669"]} start={{x:0,y:0}} end={{x:1,y:0}} style={styles.importBtnGrad}><Ionicons name="cloud-upload-outline" size={17} color="#fff"/><Text style={styles.importBtnText}>Import {importRows.length} Subject{importRows.length>1?"s":""}</Text></LinearGradient></Pressable></>)}
              </ScrollView>
            )}
            {importStage==="importing"&&(
              <View style={styles.importingWrap}>
                <View style={styles.importRingWrap}><ActivityIndicator size="large" color="#34d399" style={{position:"absolute"}}/><Text style={styles.importPct}>{importProgress}%</Text></View>
                ...
                <View style={styles.progressTrack}><View style={[styles.progressFill,{width:`${importProgress}%`}]}/></View>
            ...
            {importStage==="done"&&(
              <ScrollView contentContainerStyle={{padding:24,paddingBottom:50}}>
                <View style={{alignItems:"center",marginBottom:16}}><Ionicons name={importFailed.length===0?"checkmark-circle":"warning"} size={54} color={importFailed.length===0?"#34d399":"#f59e0b"}/><Text style={styles.importingTitle}>Import Complete!</Text></View>
                <View style={styles.importStats}><View style={[styles.importStat,{borderTopColor:"#34d399"}]}><Text style={[styles.importStatNum,{color:"#34d399"}]}>{importDone}</Text><Text style={styles.importStatLabel}>Added</Text></View><View style={[styles.importStat,{borderTopColor:"#f87171"}]}><Text style={[styles.importStatNum,{color:"#f87171"}]}>{importFailed.length}</Text><Text style={styles.importStatLabel}>Failed</Text></View></View>
                <Pressable style={[styles.importBtn,{marginTop:20}]} onPress={()=>setImportModal(false)}><LinearGradient colors={["#10b981","#059669"]} start={{x:0,y:0}} end={{x:1,y:0}} style={styles.importBtnGrad}><Ionicons name="checkmark" size={17} color="#fff"/><Text style={styles.importBtnText}>Done</Text></LinearGradient></Pressable>
              </ScrollView>
            )}
          </View>
        </View>
      ...
      <Modal visible={editModal} transparent animationType="slide" onRequestClose={()=>setEditModal(false)}>
        <View style={styles.modalOverlay}>
          <View style={[styles.sheet,{maxHeight:height*0.82}]}>
            <View style={styles.handle}/>
            <View style={styles.sheetHeader}><View style={{width:36}}/><Text style={[styles.sheetTitle,{flex:1,textAlign:"center"}]}>Edit Subject</Text><Pressable onPress={()=>setEditModal(false)} style={styles.closeBtn}><Ionicons name="close" size={19} color="#64748b"/></Pressable></View>
            <ScrollView showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled" contentContainerStyle={{padding:20,paddingBottom:50}}>
              {editingSubject&&<View style={styles.editInfoBadge}><Ionicons name="information-circle" size={13} color="#a78bfa"/><Text style={styles.editInfoText}>{editingSubject.department?.match(/\(([^)]+)\)/)?.[1]||editingSubject.department?.split(" ")[0]} · Sem {editingSubject.semester}</Text></View>}
              <Text style={styles.fieldLabel}>Subject Name *</Text>
              <View style={styles.fieldRow}><Ionicons name="book-outline" size={14} color="#64748b" style={{marginRight:8}}/><TextInput style={styles.fieldInput} value={editForm.name} onChangeText={v=>setEditForm(p=>({...p,name:v}))} placeholder="Subject name" placeholderTextColor="#374151"/></View>
              <Text style={styles.fieldLabel}>Subject Code *</Text>
              <View style={styles.fieldRow}><Ionicons name="code-slash" size={14} color="#64748b" style={{marginRight:8}}/><TextInput style={styles.fieldInput} value={editForm.code} onChangeText={v=>setEditForm(p=>({...p,code:v}))} placeholder="Subject code" placeholderTextColor="#374151" autoCapitalize="characters"/></View>
              <Text style={styles.fieldLabel}>Type *</Text>
              <TypeChips value={editForm.type||"Theory"} onChange={v=>setEditForm(p=>({...p,type:v}))}/>
              <Text style={styles.fieldLabel}>Credits</Text>
              <View style={styles.fieldRow}><Ionicons name="star-outline" size={14} color="#64748b" style={{marginRight:8}}/><TextInput style={styles.fieldInput} value={editForm.credits} onChangeText={v=>setEditForm(p=>({...p,credits:v}))} placeholder="e.g. 4" placeholderTextColor="#374151" keyboardType="numeric"/></View>
              <Pressable style={[styles.nextBtn,{marginTop:20},saving&&{opacity:0.6}]} onPress={handleEditSave} disabled={saving}><LinearGradient colors={["#f59e0b","#d97706"]} start={{x:0,y:0}} end={{x:1,y:0}} style={styles.nextBtnGrad}>{saving?<ActivityIndicator color="#fff"/>:<><Ionicons name="save-outline" size={17} color="#fff"/><Text style={styles.nextBtnText}>Save Changes</Text></>}</LinearGradient></Pressable>
            </ScrollView>
          </View>
        </View>
      </Modal>
...
const styles = StyleSheet.create({
  ...
  // Fixed filter bar
  filterBar:      { backgroundColor:"#080d17", paddingHorizontal:16, paddingTop:8, paddingBottom:8, borderBottomWidth:1, borderBottomColor:"rgba(255,255,255,0.05)" },
  collegeBadge:   { flexDirection:"row", alignItems:"center", gap:6, backgroundColor:"rgba(52,211,153,0.08)", marginBottom:8, padding:8, borderRadius:10, borderWidth:1, borderColor:"rgba(52,211,153,0.2)" },
  collegeBadgeText:{ color:"#34d399", fontSize:11, fontWeight:"600", flex:1 },
  searchBar:      { flexDirection:"row", alignItems:"center", gap:8, backgroundColor:"#1a2535", borderRadius:12, paddingHorizontal:12, paddingVertical:2, borderWidth:1, borderColor:"rgba(255,255,255,0.06)", marginBottom:8 },
  ...
  semTypeRow:     { flexDirection:"row", alignItems:"center", gap:8, marginTop:6 },
  semChip:        { paddingHorizontal:10, paddingVertical:5, borderRadius:16, borderWidth:1, borderColor:"rgba(255,255,255,0.08)", backgroundColor:"rgba(255,255,255,0.04)" },
  typeFilterRow:  { flexDirection:"row", gap:5 },
  typeFilterBtn:  { flexDirection:"row", alignItems:"center", gap:4, paddingHorizontal:9, paddingVertical:6, borderRadius:10, borderWidth:1, borderColor:"rgba(255,255,255,0.08)", backgroundColor:"rgba(255,255,255,0.04)" },
  typeFilterText: { color:"#64748b", fontSize:10, fontWeight:"700" },
