...
export default function AttendanceDetail() {
  ...
  const subjectType = (params?.subjectType || "theory").toLowerCase();
  const isBoth      = subjectType === "both";

  // ── Tab state — only relevant when isBoth ──────────
  const [activeTab, setActiveTab] = useState("theory"); // "theory" | "lab"

  // ── Theory data ────────────────────────────────────
  const [theoryRecords, setTheoryRecords] = useState([]);
  const [theoryStats,   setTheoryStats]   = useState({ total:0, present:0, absent:0, percentage:0 });

  // ── Lab data ───────────────────────────────────────
  const [labRecords, setLabRecords] = useState([]);
  const [labStats,   setLabStats]   = useState({ total:0, present:0, absent:0, percentage:0 });
  ...
  const loadDetails = async () => {
    try {
      ...
      // Always load theory
      const tRes = await API.get(`/attendance/my/subject/${subjectId}?type=theory`);
      setTheoryRecords(tRes.data?.records || []);
      setTheoryStats(tRes.data?.stats || { total:0, present:0, absent:0, percentage:0 });

      // Load lab only if Both type
      if (isBoth) {
        const lRes = await API.get(`/attendance/my/subject/${subjectId}?type=lab`);
        setLabRecords(lRes.data?.records || []);
        setLabStats(lRes.data?.stats || { total:0, present:0, absent:0, percentage:0 });
      }
    } catch (e) {
  ...
  const records = activeTab === "lab" ? labRecords : theoryRecords;
  const stats   = activeTab === "lab" ? labStats   : theoryStats;
      ...
      <View style={[styles.row, isPresent ? styles.rowPresent : styles.rowAbsent]}>
        ...
        <View style={styles.dateBlock}>
          ...
          <Text style={styles.dateMon}>
            {item.date ? new Date(item.date).toLocaleString("en-IN", { month:"short" }) : ""}
          </Text>
        </View>
        ...
        <View style={[styles.statusBadge, { backgroundColor:statusColor+"15", borderColor:statusColor+"40" }]}>
          <Ionicons name={isPresent ? "checkmark-circle" : "close-circle"} size={15} color={statusColor} />
  ...
  return (
    <View style={styles.container}>
      ...
      >
        <Pressable onPress={() => router.push("/student/attendance")} style={styles.backBtn}>
      ...
      <FlatList
        ...
        ListHeaderComponent={() => (
          <>
            {/* ═══ THEORY / LAB TABS — only for Both type ═══ */}
            {isBoth && (
              <View style={styles.tabBar}>
                {[
                  { key:"theory", label:"Theory",        icon:"book-outline",  color:"#00c6ff",
                    pct: theoryStats.percentage, present: theoryStats.present, total: theoryStats.total },
                  { key:"lab",    label:"Lab / Practical", icon:"flask-outline", color:"#34d399",
                    pct: labStats.percentage, present: labStats.present, total: labStats.total },
                ].map(tab => {
                  const isActive = activeTab === tab.key;
                  const tColor   = getColor(tab.pct);
                  return (
                    <Pressable
                      key={tab.key}
                      onPress={() => { setActiveTab(tab.key); setFilter("all"); }}
                      style={[styles.tabBtn, isActive && { borderColor:tab.color+"66", backgroundColor:tab.color+"10" }]}
                    >
                      <View style={[styles.tabIconBox, { backgroundColor:isActive?tab.color+"25":"rgba(255,255,255,0.04)" }]}>
                        <Ionicons name={tab.icon} size={16} color={isActive?tab.color:"#374151"} />
                      </View>
                      <View style={{ flex:1 }}>
                        <Text style={[styles.tabLabel, isActive && { color:tab.color }]}>{tab.label}</Text>
                        <Text style={[styles.tabPct, { color: tColor }]}>{tab.pct}%</Text>
                        <Text style={styles.tabStat}>{tab.present}/{tab.total} present</Text>
                      </View>
                      {isActive && <View style={[styles.tabDot, { backgroundColor:tab.color }]} />}
                    </Pressable>
                  );
                })}
              </View>
            ...
            <View style={styles.summaryCard}>
              <View style={[styles.percentCircle, { borderColor: pColor }]}>
                ...
                <Text style={styles.percentLabel}>
                  {isBoth ? (activeTab==="lab" ? "Lab" : "Theory") : "Attendance"}
                </Text>
              </View>
                ...
                <View style={styles.warningBox}>
                  ...
                  <Text style={styles.warningText}>
                    Below 75%!{classesNeeded()
                      ? ` Need ${classesNeeded()} more ${activeTab === "lab" ? "lab" : ""} classes`
                      : " Attendance shortage!"}
                  </Text>
                </View>
              ) : (
            ...
            <View style={styles.filterRow}>
              {[
                ...
                <Pressable
                  key={f.key}
                  style={[styles.filterBtn, filter===f.key && { backgroundColor:f.color+"18", borderColor:f.color }]}
            ...
            {filtered.length > 0 && (
              <Text style={styles.sectionTitle}>
                {filter==="all" ? "All Records" : filter==="present" ? "Present Days" : "Absent Days"}
                {isBoth ? ` · ${activeTab==="lab" ? "Lab" : "Theory"}` : ""} ({filtered.length})
              </Text>
            )}
          </>
        )}
        ...
        ListEmptyComponent={() => (
          <View style={styles.emptyState}>
            ...
            <Text style={styles.emptySub}>
              {filter === "all"
                ? `No ${isBoth ? (activeTab==="lab"?"lab":"theory")+" ":""} attendance records yet`
                : `No ${filter} records found`}
            </Text>
          </View>
        )}

        renderItem={({ item, index }) => <RecordRow item={item} index={index} />}
      />
    </View>
  );
}
...
const styles = StyleSheet.create({
  ...
  // ── Theory / Lab Tabs ──
  tabBar:          { flexDirection:"row", gap:10, marginHorizontal:16, marginTop:14, marginBottom:4 },
  ...
  tabIconBox:      { width:36, height:36, borderRadius:11, justifyContent:"center", alignItems:"center" },
  tabLabel:        { color:"#64748b", fontSize:13, fontWeight:"800" },
  tabPct:          { fontSize:16, fontWeight:"900", marginTop:1 },
  tabStat:         { color:"#374151", fontSize:10, marginTop:1 },
