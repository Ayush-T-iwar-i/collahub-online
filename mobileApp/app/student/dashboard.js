...
export default function StudentDashboard() {
  ...
  const loadAll = async (isRefresh = false) => {
    ...
    try { const r = await API.get("/dashboard/student"); if (r.data) setStats(r.data); } catch (e) {}
    try { const r = await API.get("/api/posts"); setPosts(r.data?.posts || r.data || []); } catch (e) {}
    setCheckingAuth(false);
    setRefreshing(false);
  };

  // ✅ Dashboard → Double back = App close
  useFocusEffect(
    useCallback(() => {
      const backAction = () => {
...
const styles = StyleSheet.create({
  ...
  postAvatar: { width: 42, height: 42, borderRadius: 21, justifyContent: "center", alignItems: "center", marginRight: 10 },
  ...
  postFooter: { flexDirection: "row", alignItems: "center", gap: 16, marginTop: 14, paddingTop: 12, borderTopWidth: 1, borderTopColor: "rgba(255,255,255,0.05)" },
