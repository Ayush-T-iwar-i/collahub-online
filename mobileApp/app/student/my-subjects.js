...
export default function StudentMySubjects() {
  ...
  const loadAll = async (isRefresh = false) => {
    ...
    try {
      const [subRes, ttRes] = await Promise.all([
        ...
        API.get("/subject-requests/student-timetable").catch(() => ({
          data: { timetable: {}, slots: [] },
        })),
      ]);

      setSubjects(subRes.data?.subjects || []);
      // subjectController returns "info" not "student"
      setStudentInfo(subRes.data?.info || null);
  ...
  // Today's next class
  const todaySlots = (timetable[todayName] || []).sort((a, b) =>
              ...
              ListEmptyComponent={() => (
                <View style={styles.empty}>
                  ...
                  <Text style={styles.emptySub}>
                    Admin has not added subjects for your college, department and semester yet.
                  </Text>
                </View>
              )}
            ...
            <ScrollView
              ...
              {filteredDays.length === 0 ? (
                <View style={styles.emptyTT}>
                  ...
                  <Text style={styles.emptySub}>
                    {activeDay === "All"
                      ? "Teachers have not set their schedules yet."
