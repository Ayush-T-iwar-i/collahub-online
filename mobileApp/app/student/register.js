...
// ── Auto Semester Calculator ──
const getAutoSemester = (admissionYear) => {
  if (!admissionYear || admissionYear.length < 4) return null;
...
export default function StudentRegister() {
  ...
  const [form, setForm] = useState({
    ...
    // ✅ studentId NAHI — backend auto generate karega
  });
  ...
  const handleRegister = async () => {
    const { name, phone, admissionYear, college, department, gender, password, confirmPassword } = form;
    ...
    if (admissionYear.length !== 4) return Alert.alert("Error", "Enter valid 4 digit year");
    ...
    try {
      ...
      const res = await API.post("/auth/register", {
        ...
        // ✅ studentId nahi bhej rahe — backend auto generate karega
      });

      // ✅ Generated Student ID show karo
      const generatedId = res.data?.student?.studentId || res.data?.studentId;

      Alert.alert(
        "Registration Successful! 🎉",
        `Your Student ID:\n\n📛 ${generatedId || "Will be assigned"}\n\nPlease login to continue.`,
  ...
  const autoSem = getAutoSemester(form.admissionYear);
          ...
          <BlurView intensity={70} tint="dark" style={styles.blurInner}>
            ...
            {step === 1 && (
              <>
                ...
                <Pressable
                  ...
                  disabled={sendingOtp}
                >
                  <LinearGradient colors={["#0072ff", "#00c6ff"]} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }} style={styles.btnGrad}>
                    {sendingOtp
                ...
                <Pressable onPress={() => router.replace("/(auth)/student-login")} style={styles.loginLink}>
                  <Text style={styles.loginLinkText}>Already have an account? <Text style={{ color: "#00c6ff" }}>Login</Text></Text>
                </Pressable>
              </>
            )}
            ...
            {step === 2 && (
              <>
                ...
                <View style={styles.otpRow}>
                  {otp.map((digit, index) => (
                    <TextInput
                      ...
                      style={[styles.otpBox, digit && { borderColor: "#00c6ff", backgroundColor: "rgba(0,198,255,0.08)" }]}
                ...
                <View style={styles.dotsRow}>
                  {otp.map((d, i) => <View key={i} style={[styles.dot, d && styles.dotFilled]} />)}
                </View>
                <Text style={[styles.timerText, otpExpired && { color: "#f87171" }]}>
                  {otpExpired ? "⚠️ OTP Expired" : `Expires in ${formatTime()}`}
                </Text>
                {!otpExpired ? (
                  <Pressable style={[styles.btn, (verifyingOtp || filled < 6) && { opacity: 0.6 }]} onPress={handleVerifyOtp} disabled={verifyingOtp || filled < 6}>
                    <LinearGradient colors={filled === 6 ? ["#0072ff", "#00c6ff"] : ["#1a2535", "#1a2535"]} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }} style={styles.btnGrad}>
                      {verifyingOtp
                        ? <ActivityIndicator color="#fff" />
            ...
            {step === 3 && (
              <>
                ...
                <Field icon="calendar-outline" placeholder="Admission Year (e.g. 2024)" value={form.admissionYear}
                  onChangeText={(t) => setForm({ ...form, admissionYear: t.replace(/[^0-9]/g, "") })}
                  keyboardType="numeric" maxLength={4} />

                {/* ✅ Auto Semester Preview */}
                {autoSem && (
                  <View style={styles.autoSemBadge}>
                    <Ionicons name="school" size={14} color="#34d399" />
                    <Text style={styles.autoSemText}>
                      Auto Semester: <Text style={{ color: "#34d399", fontWeight: "800" }}>Sem {autoSem}</Text>
                    </Text>
                  </View>
                )}

                {/* ✅ Auto Student ID Preview */}
                {form.admissionYear?.length === 4 && form.department && (
                  <View style={styles.autoIdBadge}>
                    <Ionicons name="card" size={14} color="#a78bfa" />
                    <Text style={styles.autoIdText}>
                      Student ID will be: <Text style={{ color: "#a78bfa", fontWeight: "800" }}>
                        {form.admissionYear}-{form.department.match(/\(([^)]+)\)/)?.[1] || form.department.split(" ")[0]}-XXX
                      </Text> (auto assigned)
                    </Text>
                  </View>
                )}
                ...
                <Pressable style={[styles.btn, { marginTop: 8 }, registering && { opacity: 0.7 }]} onPress={handleRegister} disabled={registering}>
      ...
      <Modal visible={collegeModal} transparent animationType="slide" onRequestClose={() => setCollegeModal(false)}>
        <Pressable style={styles.modalOverlay} onPress={() => setCollegeModal(false)}>
          <View style={styles.modalSheet}>
            ...
            <FlatList
              data={COLLEGES} keyExtractor={(item) => item}
              renderItem={({ item }) => (
                <Pressable
                ...
                >
                  <View style={styles.modalOptionLeft}>
                    ...
                    <Text style={[styles.modalOptionText, form.college === item && { color: "#00c6ff" }]} numberOfLines={2}>{item}</Text>
                  </View>
        ...
        <Pressable style={styles.modalOverlay} onPress={() => setDeptModal(false)}>
          <View style={styles.modalSheet}>
            ...
            <View style={styles.searchBar}>
              ...
              <TextInput placeholder="Search department..." placeholderTextColor="#374151"
                style={styles.searchInput} value={deptSearch} onChangeText={setDeptSearch} />
            </View>
            <FlatList
              data={filteredDepts} keyExtractor={(item) => item}
              renderItem={({ item }) => (
                <Pressable
                ...
                >
                  <View style={styles.modalOptionLeft}>
                    ...
                    <Text style={[styles.modalOptionText, form.department === item && { color: "#00c6ff" }]} numberOfLines={2}>{item}</Text>
                  </View>
...
const styles = StyleSheet.create({
  ...
  // ✅ Auto Sem Badge
  autoSemBadge: { flexDirection: "row", alignItems: "center", gap: 8, backgroundColor: "rgba(52,211,153,0.08)", padding: 10, borderRadius: 10, marginBottom: 10, borderWidth: 1, borderColor: "rgba(52,211,153,0.2)" },
  autoSemText: { color: "#94a3b8", fontSize: 12 },
  // ✅ Auto ID Badge
  autoIdBadge: { flexDirection: "row", alignItems: "center", gap: 8, backgroundColor: "rgba(167,139,250,0.08)", padding: 10, borderRadius: 10, marginBottom: 10, borderWidth: 1, borderColor: "rgba(167,139,250,0.2)" },
  autoIdText: { color: "#94a3b8", fontSize: 11, flex: 1 },
