...
export default function StudentAttendance() {
  ...
  // ✅ Modal hataya — direct page pe navigate
  const openDetail = (subject) => {
    router.push({
      pathname: `/student/attendance/${encodeURIComponent(subject.subjectName)}`,
      params: {
          ...
          {item.percentage < 75 && (
            <View style={styles.warningRow}>
              ...
              <Text style={styles.warningText}>
                {Math.ceil((75 * item.total - 100 * item.present) / 25)} more classes attend karo
              </Text>
            </View>
          )}
        </View>
      ...
      {summary.length > 0 && (
        <View style={styles.overallBox}>
          ...
          <View style={styles.overallStats}>
            ...
            <Text style={styles.overallStatSub}>
              {summary.filter(s => s.percentage < 75).length} subjects ⚠️ below 75%
            </Text>
            {overallPct < 75 && (
        ...
        <FlatList
          ...
          ListEmptyComponent={() => (
            <View style={styles.emptyState}>
              ...
              <Text style={styles.emptySub}>Teacher ne abhi attendance nahi lagayi</Text>
            </View>
          )}
